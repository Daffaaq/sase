...
@section('container')
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Kategori Surat Masuk Management</h6>
        </div>
        <div class="card-body">
            <button id="createCategoryBtn" class="btn btn-success float-right mb-3">
                <i class="fas fa-plus"></i> Create Kategori Surat Masuk
            </button>
            <div class="table-responsive">
                <table class="table table-bordered" id="categoryTable" width="100%" cellspacing="0">
                    <thead>
                        <tr>
                            <th>No</th>
                            <th>Nama Kategori Surat Masuk</th>
                            <th>Action</th>
                        </tr>
                    </thead>
                    <tbody></tbody>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="categoryModalLabel">Create Kategori Surat Masuk</h5>
                    <button type="button" id="closeCategoryModal" class="btn-close" data-bs-dismiss="modal"
                        aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="categoryForm" action="{{ route('Kategori-Surat-Masuk-Kadiv.store') }}" method="POST">
                        @csrf
                        <div id="success-message" class="alert alert-success" style="display:none;">Data Berhasil Dikirim
                        </div>
                        <div id="error-message" class="alert alert-danger" style="display:none;"></div>
                        <div class="mb-3">
                            <label for="name_jenis_surat_masuk" class="form-label">Nama Kategori Surat Masuk</label>
                            <input type="text" class="form-control" id="name_jenis_surat_masuk"
                                name="name_jenis_surat_masuk" required>
                        </div>
    ...
    <script>
        $(document).ready(function() {
            var dataMaster = $('#categoryTable').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: '{{ route('kategori-surat-masuk-list-kadiv') }}',
                    type: 'POST',
                    dataType: 'json',
                    data: {
                        _token: '{{ csrf_token() }}'
                ...
                columns: [{
                    ...
                    {
                        data: 'name_jenis_surat_masuk',
                        name: 'name_jenis_surat_masuk'
                    },
                    {
                        data: 'uuid',
                        name: 'uuid',
                    ...
                    <button data-uuid="${row.uuid}" data-name="${row.name_jenis_surat_masuk}" class="btn icon btn-sm btn-danger deleteCategoryBtn" data-bs-toggle="tooltip" title="Delete Category">
            ...
            $('#createCategoryBtn').click(function() {
                ...
                $('#categoryModalLabel').text('Create Kategori Surat Masuk');
                $('#categoryModal').modal('show');
            });

            $('#categoryTable').on('click', '.editCategoryBtn', function() {
                var uuid = $(this).data('uuid');
                var url = '{{ route('Kategori-Surat-Masuk-Kadiv.edit', ':uuid') }}';
                url = url.replace(':uuid', uuid);

                $.get(url, function(response) {
                    var data = response.data;
                    $('#categoryModalLabel').text('Edit Kategori Surat Masuk');
                    var updateUrl = '{{ route('Kategori-Surat-Masuk-Kadiv.update', ':uuid') }}';
                    ...
                    $('#name_jenis_surat_masuk').val(data.name_jenis_surat_masuk);
                    $('#categoryModal').modal('show');
                }).fail(function(jqXHR) {
            ...
            $('#confirmDeleteBtn').click(function() {
                var uuid = $(this).data('uuid');
                var url = '{{ route('Kategori-Surat-Masuk-Kadiv.destroy', ':uuid') }}';
                url = url.replace(':uuid', uuid);
